...
const menuBarHeight = document.querySelector('#menu').getBoundingClientRect().height;
...
// Rolamento // 
document.querySelector('.menu-holder').addEventListener('click', function(elem){
  elem.preventDefault();
...
document.querySelector('#header-content').addEventListener('click', function(elem){
  elem.preventDefault();
  if(elem.target.id){
    const id = elem.target.getAttribute('href');
    document.querySelector(id).scrollIntoView({behavior: "smooth"});
